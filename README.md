...
- **vs. `@octokit/webhooks`** - that's a webhook *framework* (event routing,
  handler registration, a lot of GitHub-specific shape). This is the
  verification primitive underneath, without the framework, and works for
  Stripe-style, Twilio, and Mandrill too. Reach for octokit when you want the
  framework; reach for this when you want the four lines that actually check
  the signature.
- **vs. writing it yourself** - the mistakes are old and consistent:
  verifying re-serialized JSON instead of the raw bytes, using `==` against
  an attacker-supplied signature, forgetting the timestamp tolerance is
  bidirectional (future clock skew is also an attack), reconstructing the
  Twilio URL from `req.url` behind a proxy. Each of those has a test in this
  library's suite specifically because I've seen it in the wild.
- **vs. provider-specific packages** - `stripe`, `twilio`, and
  `@mailchimp/mandrill` all ship verifiers as part of much larger SDKs. If
  you only need to *verify* (a webhook receiver, not an API client), one
  ~170-LOC zero-dep package for all four providers is a lighter dependency
  than three SDKs.
